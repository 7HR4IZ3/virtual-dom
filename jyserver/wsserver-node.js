...
class ClientContext {
          ...
          return _this.proxymise(
            new Promise((resolve, reject) => {
              ...
              .then((result) => {
                ...
                if (result == undefined) {
                  if (prop !== "then") {
                    return reject("No attribute name: " + prop);
                  }
                } else {
                  resolve(result);
                }
          ...
          return _this.proxymise(
            new Promise((r, rj) => {
              if (property.endsWith("_")) {
                return r(_this.formatters.callable_proxy(property.trimEnd("_"), res, true)); 
              }
  ...
  transform(code, ...opts) {
    if (this.transformer == "babel" && babel) {
  ...
  connect() {
    ...
    // console.log(this.backend.getState())
  }
          ...
          return new Promise((resolve, reject) => {
            ...
            _this
              .recieve({
                ...
                prop: property.endsWith("_") ? property.trimEnd("_") : property,
                target: res.location,
              })
              .then((result) => {
                // console.log("\nResult: ", result);
                try {
                  if (property.endsWith("_") && result.location) {
                    return resolve(_this.formatters.callable_proxy(property.trimEnd("_"), result, true)); 
                  }
                  result = _this.get_result(result, property);
                } catch (error) {
                  return undefined;
