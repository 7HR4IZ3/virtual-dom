...
class ClientContext:
    ...
    def __init__(self, cls, uid=None, verbose=False, custom_tasks=None):
        self.appClass = cls
        self.obj = cls()
        self.queries = {}
    ...
    def processCommand(self, req):
        ...
        if task == "state":
            ...
            t = self.custom_tasks.get(task, None)
            if t:
                return t(req)
            return ""
...
class JSchain:
    ...
    def setdata(self, attr, value, aprepend="."):
        ...
        if callable(value):
            ...
            self._add(attr, prepend=aprepend)
            self._add("=" + json.dumps(value), prepend="")
        return value
    ...
    def eval(self, timeout=10):
        ...
        try:
            ...
            try:
                ...
                if c.socket:
                    result = json.loads(c.socket.receive())
                else:
...
class JSroot:
    ...
    def createClass(self, name, body):
        if isinstance(body, dict):
            body = self.createClassMethods(body)
    ...
    def createFunction(self, name, code):
        pass
